...
No real gPTP captures are readily available (unlike pcapng ones), so this hand-builds one covering all
8 message kinds, a Follow_Up Information TLV, and an Announce PATH_TRACE TLV with 3 entries — reusing
the same minimal pcapng SHB/IDB/EPB layout the rest of this repo's examples already use. EXPECTED holds
every value used to construct the messages, so test_gptp.py asserts against ground truth it didn't
duplicate by hand.

    python build_fixture.py [out.pcapng]     # default: gptp_fixture.pcapng next to this script
"""
import pathlib
import struct
import sys
