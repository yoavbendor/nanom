...
// gPTP (IEEE 802.1AS) row structs — the "your format" half of the nanom Python-bindings pattern.
//
...
// in gptp_parse.hpp passes std::endian::big explicitly rather than a file-derived order.
#pragma once
...
// Every row carries these common-header fields flattened in, playing the role of nanolance's
// packet_id join key (msg_index is assigned by the parser, not read from the wire).
struct RowCommon {
...
NANOM_DESCRIBE(nmgptp::RowCommon, msg_index, domain_number, sequence_id, correction_field,
