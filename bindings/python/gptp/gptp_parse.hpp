...
// gPTP (IEEE 802.1AS) dispatch + TLV walking, built on nanom.
//
...
inline RowCommon make_common(std::uint64_t msg_index, const GptpHeader& h) {
  RowCommon c;
...
inline bool parse_sync(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
  SyncRow row{};
  row.common = common;
...
inline bool parse_delay_req(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
  DelayReqRow row{};
  row.common = common;
  auto ts = read_timestamp(fr);
...
inline bool parse_pdelay_req(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
  PdelayReqRow row{};
  row.common = common;
  auto ts = read_timestamp(fr);
...
inline bool parse_delay_resp(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
  DelayRespRow row{};
  row.common = common;
  auto ts = read_timestamp(fr);
...
inline bool parse_pdelay_resp(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
  PdelayRespRow row{};
  row.common = common;
  auto ts = read_timestamp(fr);
...
inline bool parse_pdelay_resp_follow_up(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
...
inline bool parse_follow_up(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
  FollowUpRow row{};
  row.common = common;
  auto ts = read_timestamp(fr);
...
inline bool parse_announce(GptpTables& t, const RowCommon& common, nm::input body) {
  FieldReader fr{body};
  AnnounceRow row{};
  row.common = common;
  auto ts = read_timestamp(fr);
...
inline bool parse_gptp_message(GptpTables& t, nm::input msg) {
  ...
  const RowCommon common = make_common(t.next_msg_index++, h);
  switch (std::uint8_t(h.message_type)) {
    case kMsgSync:               return parse_sync(t, common, body);
    case kMsgDelayReq:            return parse_delay_req(t, common, body);
    case kMsgPdelayReq:           return parse_pdelay_req(t, common, body);
    case kMsgPdelayResp:          return parse_pdelay_resp(t, common, body);
    case kMsgFollowUp:            return parse_follow_up(t, common, body);
    case kMsgDelayResp:           return parse_delay_resp(t, common, body);
    case kMsgPdelayRespFollowUp:  return parse_pdelay_resp_follow_up(t, common, body);
    case kMsgAnnounce:            return parse_announce(t, common, body);
    default: return false;  // Signaling/Management/reserved — out of scope for this example
  }
}

// ---- pcapng EPB scanning (the same minimal, self-contained shape as example_pcapng.cpp) + Ethernet
// demux: gPTP runs directly over Ethernet (ethertype 0x88F7), so each EPB's captured packet data is
// checked for that ethertype and, if it matches, the bytes after the 14-byte Ethernet header are handed
// to parse_gptp_message. Ethernet/gPTP bytes are always network byte order regardless of the pcapng
// section's own endianness, so this uses std::endian::big explicitly rather than the section's order.
namespace detail {
struct EthHdr { std::array<std::uint8_t, 6> dst, src; std::uint16_t ethertype; };
...
NANOM_DESCRIBE(nmgptp::detail::EthHdr, dst, src, ethertype);
NANOM_DESCRIBE(nmgptp::detail::png_block_hdr, type, total_len);
NANOM_DESCRIBE(nmgptp::detail::png_epb_body, interface_id, ts_high, ts_low, caplen, origlen);
namespace nmgptp {
inline constexpr std::uint32_t kEtherTypeGptp = 0x88F7U;
inline constexpr std::uint32_t kPngShb = 0x0A0D0D0AU, kPngEpb = 6, kPngByteOrderMagic = 0x1A2B3C4DU;
inline constexpr std::endian png_order_of(bool little) { return little ? std::endian::little : std::endian::big; }

inline GptpTables parse_pcapng_with_gptp(const std::uint8_t* data, std::size_t n) {
  GptpTables tables;
  bool little = true;
  std::size_t pos = 0;
  while (pos + 8 <= n) {
    auto hdr = nm::strct<detail::png_block_hdr>(png_order_of(little))(nm::from(data + pos, n - pos));
    if (!hdr) break;
    std::uint32_t type = hdr->value.type, total = hdr->value.total_len;
    if (type == kPngShb) {
      if (pos + 12 > n) break;
      std::uint32_t bom;
      std::memcpy(&bom, data + pos + 8, 4);
      little = (bom == kPngByteOrderMagic);
      auto h2 = nm::strct<detail::png_block_hdr>(png_order_of(little))(nm::from(data + pos, n - pos));
      if (!h2) break;
      total = h2->value.total_len;
    }
    if (total < 12 || total % 4 != 0 || pos + total > n) break;
    if (type == kPngEpb) {
      auto e = nm::strct<detail::png_epb_body>(png_order_of(little))(nm::from(data + pos + 8, total - 8));
      if (e) {
        const std::uint32_t caplen = e->value.caplen;
        const std::uint8_t* pd = data + pos + 28;  // packet data starts after the 20-byte EPB fixed body
        if (caplen >= 14 && 28 + std::size_t(caplen) <= total) {
          auto eth = nm::strct<detail::EthHdr>(std::endian::big)(nm::from(pd, caplen));
          if (eth && eth->value.ethertype == kEtherTypeGptp) {
            parse_gptp_message(tables, eth->rest);
          }
        }
      }
    }
